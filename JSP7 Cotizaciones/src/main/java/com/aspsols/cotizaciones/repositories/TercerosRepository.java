...
public class TercerosRepository {
	...
	public List<Terceros> findByEmpresa(String empresa, String filter) {
		return jdbcTemplate.query("SELECT * FROM ("
				+ " SELECT N.C_EMP C_EMP, N.N_IDE N_IDE, N.NOM NOMBRE, C.IVA IVA, N.ZONA"  
				+ " FROM CLIENTE C" 
				+ " JOIN NITS N ON C.N_IDE = N.N_IDE AND C.C_EMP = N.C_EMP"  
				+ " LEFT JOIN PROSP_CL P ON C.N_IDE = P.N_IDE AND C.C_EMP = P.C_EMP"  
				+ " WHERE P.N_IDE IS NULL" 
				+ " UNION "  
				+ " SELECT P.C_EMP C_EMP, P.N_IDE N_IDE, P.NOMBRE NOMBRE, P.IVA IVA, N.ZONA"  
				+ " FROM PROSP_CL P" 
				+ " LEFT JOIN (CLIENTE C JOIN NITS N ON C.N_IDE = N.N_IDE AND C.C_EMP = N.C_EMP)"  
				+ " ON C.N_IDE = P.N_IDE AND C.C_EMP = P.C_EMP" 
				+ " WHERE C.N_IDE IS NULL)"
	...
	public List<Terceros> findByNit(String empresa, String nit) {
		return jdbcTemplate.query("SELECT * FROM ("
				...
				+ " WHERE P.N_IDE IS NULL" 
				+ " UNION "  
				+ " SELECT P.C_EMP C_EMP, P.N_IDE N_IDE, P.NOMBRE NOMBRE, P.IVA IVA, P.ZONA"  
				+ " FROM PROSP_CL P" 
				+ " LEFT JOIN (CLIENTE C JOIN NITS N ON C.N_IDE = N.N_IDE AND C.C_EMP = N.C_EMP)"  
				+ " ON C.N_IDE = P.N_IDE AND C.C_EMP = P.C_EMP" 
				+ " WHERE C.N_IDE IS NULL)"
	...
	public void insertProspecto(Terceros entity) {
		String sql = "insert into PROSP_CL(C_EMP,N_IDE,NOMBRE,ZONA,IVA)"
				+ " values(?,?,?,?,?)";
	...
	public void updateProspecto(Terceros entity) {
		String sql = "UPDATE PROSP_CL SET NOMBRE = ?, ZONA = ?, IVA = ?"
				+ " where N_IDE = ? AND C_EMP = ?";
		jdbcTemplate.update(new PreparedStatementCreator() {
			@Override
			public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
				//PreparedStatement ps = connection.prepareStatement(sqlEnc, Statement.RETURN_GENERATED_KEYS);
	...
	public void deleteProspecto(Terceros entity) {
		String sql = "delete FROM PROSP_CL"
				+ " where N_IDE = ? AND C_EMP = ?";
		jdbcTemplate.update(new PreparedStatementCreator() {
			@Override
			public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
